...
instead of through `serve_forever`'s signal-handling loop), measure it with the
same attach-mode prober (B2) against the manifest that boot just wrote, then
tear down before the next cell. Model-load time is why cells are grouped by
stack rather than interleaved.
"""
...
async def run_cell_cleanroom(
    job: Job, spec: BenchSpec, cell_state_dir: str | Path
) -> dict[str, Any]:
...
def make_cleanroom_executor(spec: BenchSpec, state_dir: str | Path) -> CellExecutor:
    ...
    base_dir = Path(state_dir) / "benchmarks" / "_cleanroom"

    def executor(job: Job) -> dict[str, Any]:
        cell_state_dir = base_dir / job.cell_key
        return asyncio.run(run_cell_cleanroom(job, spec, cell_state_dir))
